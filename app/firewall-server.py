...
mcp = FastMCP("ItsNotTheFirewall",
              instructions="""
              This server provides access to the Ansible Automation Platform API.
              It launches a playbook template that gathers ACL data from the device.
              The playbook is launched using the Ansible Automation Platform API.
              The server will return the ACL data in JSON format.
              """)


# Define the payload for the playbook launch (if any extra variables are needed) for future use
payload = {
    "extra_vars": {
        "dut": "eda-sw100"
    }
...
# Define the headers necessary for the API request to Ansible Automation Platform
headers = {
...
def launch_template():
    ...
    url = api_url + f"/api/controller/v2/job_templates/39/launch/"
